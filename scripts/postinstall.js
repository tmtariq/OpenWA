 ...
 *   9. `node scripts/patch-wwebjs-media-send.js --best-effort` when present, avoiding mediaOptions
 *      model pollution when sending media messages, gated the same way.
...
function planSteps(root, env = process.env) {
  ...
  const mediaSendPatcher = path.join(root, 'scripts', 'patch-wwebjs-media-send.js');
  if (fs.existsSync(mediaSendPatcher)) {
    steps.push({
      name:
        'whatsapp-web.js media send model pollution repair ' +
        '(scripts/patch-wwebjs-media-send.js --best-effort)',
      command: process.execPath,
      args: [mediaSendPatcher, '--best-effort'],
      options: { stdio: 'inherit', cwd: root, env: cleanEnv },
    });
  }
